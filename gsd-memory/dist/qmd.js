...
export const qmd = {
     ...
     */
    async qmdSearch(options) {
        try {
            const limit = options.limit || 10;
            const { stdout } = await execAsync(`qmd query "${options.query}" --collection "${options.collection}" --limit ${limit} --json`);
    ...
    async createCollection(options) {
        ...
        try {
            ...
            await execAsync(`qmd add "${options.name}" ${paths} ${contextArg}`);
            return { success: true };
        }
        catch (error) {
            return {
