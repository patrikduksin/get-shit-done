...
import { extractProject } from '../extractors/project.js';
import { readFileSync, existsSync, readdirSync, statSync } from 'fs';
import { join } from 'path';
/**
 * Search for decisions across all registered GSD projects
 */
export async function findDecisions(options) {
    ...
    if (options.query) {
        ...
        filtered = allDecisions.filter(d => d.decision.toLowerCase().includes(query) ||
            d.rationale?.toLowerCase().includes(query));
    }
    // Filter by tags if provided
    if (options.tags && options.tags.length > 0) {
        const tags = options.tags.map(t => t.toLowerCase());
        filtered = filtered.filter(d => d.tags?.some(t => tags.includes(t.toLowerCase())));
    }
    // Apply limit
 ...
 * Extract decisions from PROJECT.md
 */
function extractProjectDecisions(planningPath, projectName) {
 ...
 */
function extractSummaryDecisions(planningPath, projectName) {
    const decisions = [];
 ...
 */
function findSummaryFiles(dir, files = []) {
    try {
