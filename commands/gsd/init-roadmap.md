...
description: Configure workflow, research domain, define requirements, and create roadmap
allowed-tools:
  - Read
  - Bash
  - Write
...
Continue project initialization: workflow config → research (optional) → requirements → roadmap.

Run this after `/gsd:new-project` creates PROJECT.md. This command handles the file-driven phases that don't require the questioning context.
...
**MANDATORY FIRST STEP — Verify PROJECT.md exists:**

```bash
[ ! -f .planning/PROJECT.md ] && echo "ERROR: No PROJECT.md found. Run /gsd:new-project first." && exit 1
```

**Check for partial completion (resume logic):**
...
```
Project already initialized. Use /gsd:progress to see status.
```

Exit command.

**Otherwise:** Continue, skipping completed phases:

- If `HAS_CONFIG=yes` → skip Phase 2
- If `HAS_RESEARCH=yes` → skip Phase 3
- If `HAS_REQUIREMENTS=yes` → skip Phase 4

## Phase 2: Workflow Preferences

**Skip if config.json exists.**
...
## Phase 2.5: Resolve Model Profile
...
**Skip if research/ directory exists.**
...
**Skip if REQUIREMENTS.md exists.**
...
- [ ] PROJECT.md exists (guard check passed)
